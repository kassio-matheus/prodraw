...
class SquareController(Tools):
    ...
    def _on_drag(self, event: Event):
        ...
        if self.current.has_min_size():
            ...
            # Let the View draw the standard base preview (using 'square_preview' tag)
            ...
            self.canvas.itemconfig("square_preview", **style)

    def _on_release(self, event: Event):
        """Step 3: mouse up commits the Square if it meets the minimum size."""
