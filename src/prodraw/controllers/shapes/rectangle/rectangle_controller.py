...
class RectangleController(Tools):
    ...
    def _on_drag(self, event: Event):
        ...
        if self.current.has_min_size():
            ...
            # Let the View draw the standard base preview
            self.view.draw_preview(
                self.current.start_x, self.current.start_y,
                self.current.end_x, self.current.end_y, self.current.bg
            )

            # Instantly apply the active Tool Options style to the preview tag
            style = self.get_current_shape_style()
            self.canvas.itemconfig("rectangle_preview", **style)

    def _on_release(self, event: Event):
        """Step 3: mouse up commits the rectangle if it meets the minimum size.
        The confirmed rectangle is appended to the model and drawn once."""
